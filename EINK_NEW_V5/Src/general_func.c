...
void display_fields(uint8_t* image_bw)
{
	  ...
	  epd_paint_drawLine(100,100,100,200,EPD_COLOR_BLACK,3);
	  epd_paint_drawLine(100,100,199,100,EPD_COLOR_BLACK,3);
	  epd_displayBW_partial(image_bw);
}
...
void turn_of(uint8_t *image_bw, uint8_t *text)
{
	extern turn_of_flag;
    sprintf((char*)text, "%d", "TURN OF");
    epd_paint_showString(10,100, text, 40, EPD_COLOR_BLACK);
    epd_displayBW_partial(image_bw);
    HAL_Delay(3000);

	epd_paint_drawRectangle(0, 0, 200, 200, EPD_COLOR_WHITE, 1);
	epd_displayBW_partial(image_bw);
	turn_of_flag = 0;
}
